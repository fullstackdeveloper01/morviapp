    ...
    */
    ...
    public function videoPost(Request $request){
        try{
            ...
            $data = $request->only('video', 'text', 'user_id', 'location','video_thumbnail');
            $validator = Validator::make($data, [
            ...
            if($request->hasfile('video')){
               ...
               if($request->hasfile('video_thumbnail')){
                    foreach($request->file('video_thumbnail') as $file)
                    {
                        $randomNum = rand(10,100);
                        $thb = time().$randomNum.'.'.$file->getClientOriginalExtension();
                        $thumbail[] = $thb;
                        $destinationPath = base_path('public/post');
                        $file->move($destinationPath, $thb);
                    }
                    $thumbail = implode(',', $thumbail);
                    $post['video_thumbnail'] =  $thumbail;
               }
                $post =  Post::create($post);
                if(!empty($post)){
                    return response()->json = [
    ...
    public function getPostByUserID(Request $request){
        try{
            ...
            if(!empty($request->self_id)){
              $getAllPosts = Post::select('posts.*','posts.id as post_id','posts.user_id as user_id','users.name','users.name','users.email','users.phone','users.address','users.role','users.active','users.follower_count','users.image as user_image','posts.created_at as post_created_at')
               ...
               $getAllPosts =Post::select('follow_unfollows.status as follow_status', 'follow_unfollows.post_id', 'master_cities.city_name as location_name', 'posts.*', 'users.name', 'users.email', 'users.phone', 'users.address', 'users.role', 'users.active', 'users.follower_count', 'users.image as user_image', 'posts.created_at as post_created_at')
                           ->leftJoin('users', 'users.id', '=', 'posts.user_id')
                           ->leftJoin('follow_unfollows', 'follow_unfollows.follower_id', '=', 'posts.user_id')
                           ->leftJoin('follow_unfollows as new_follow_tb', 'new_follow_tb.user_id', '=', 'posts.user_id')
                           ->leftJoin('master_cities', 'master_cities.id', '=', 'posts.location')
                           ->where(function($query) use ($request){
                             $query->orWhere('follow_unfollows.status',1);
                             $query->orWhere('new_follow_tb.status',1);
                            })
                           ->where(function ($query) use ($request){
                             $query->orWhere(['follow_unfollows.user_id' => $request->user_id]);
                             $query->orWhere(['follow_unfollows.follower_id' => $request->user_id]);
                              $query->orWhere(['new_follow_tb.user_id' => $request->user_id]);
                             $query->orWhere(['new_follow_tb.follower_id' => $request->user_id]);
                           })
                           ->orWhere('posts.user_id', $request->user_id)
                           ->groupBy('posts.id')
                           ->latest('posts.id')->paginate($request->limit); 
            ...
            foreach($getAllPosts as $gtpost){
                $likeCounts = DB::table('follow_unfollows')->where(['user_id'=> $gtpost->user_id,'post_id'=> $gtpost->id,'type' => 1])->count();
                if($likeCounts > 999){
                ...
                unset($thubnailImg);
                $thubnailImg[] = $basepath.'/'.$gtpost['video_thumbnail'];
                ...
                $data['is_like'] = $this->isFav($gtpost['user_id'],$request->login_user_id,$gtpost['id'],'1');
                $data['posted_date'] = $this->getDateInFormat($gtpost); 
                $data['follower_count'] =  $this->getCountOfFollowerAndFollowing($type = 1,$gtpost['user_id']);
                $data['following_count'] =  $this->getCountOfFollowerAndFollowing($type = 2,$gtpost['user_id']);
        ...
        try{
            ...
            if($request->hasfile('video_thumbnail')){
                ...
                {            
                    ...
                    $NewImage1 = implode(',', $thum);
                    $post['video_thumbnail'] =  $NewImage1;
               }
            $imagesP=[];
            if($request->hasfile('image')){
                foreach($request->file('image') as $file)
    ...
    public function friendsDetials(Request $request){
        try{
            ...
            $details = User::select('users.id','users.name','users.email','users.phone','users.image',             'users.address','users.place','users.business_name','users.follower_count',             'users.created_at','users.about_business','users.experience')
            ...
            // Convert null values to empty strings in the $details array
             ...
             if(count($details) > 0){
               ...
               foreach($details as $newDt){
                  $friendStatus = DB::table('follow_unfollows')->select('status','id')->where(['follower_id' => $request->login_user_id,
                    'user_id' => $request->friend_user_id,'type' => 0])->first();
                    if(!empty($friendStatus)){
                      if($friendStatus->status == 0){
                      $requestStatus = 'pending';
                    }elseif($friendStatus->status == 1){
                   ...
                   $following_count = DB::table('follow_unfollows')->select('user_id')
                                    ->where('follower_id',$userID)->where(['type' => 0 , 'status' => 1])->count();
                  $follower_count = DB::table('follow_unfollows')->select('user_id')
                  ...
                  if(!empty($friendStatus->id)){
                    $data['follow_unfollow_id'] = $friendStatus->id;
                  }else{
                    $data['follow_unfollow_id'] = '';
                  }
    ...
    public function followUnfollow(Request $request){
        try{
          ...
          if ($validator->fails()) {
           ...
           $post = $request->all();
           $newPost['user_id'] = $post['follower_id'];
           $newPost['follower_id'] = $post['user_id'];
           ...
           $followerCount = '';
           $dt = DB::table('follow_unfollows')->where(['user_id'=> $request->follower_id,'follower_id'=>$request->user_id,'type' =>0])->count();
           if($dt > 0){
            $data = DB::table('follow_unfollows')->where(['user_id'=> $request->follower_id,'follower_id'=>$request->user_id])->where('type',0)->delete();
            ...
            $data = DB::table('follow_unfollows')->insert($newPost);
            ...
            if(!is_null($getUser)){
                ...
                if($dt > 0){
                    ...
                    $disLike[] = [
                     ...
                     if(!empty($getDeviceID)){
                        if($getUser->user_id != $request->follower_id){
          ...
          $validator = Validator::make($request->all(), [
            ...
            if(!empty($request->search)){
                         $query->where('users.name', 'like', $request->search. '%')
                                ->where(['follow_unfollows.follower_id'=> $request->user_id,'type' => 0]);
            }else{
                 $query->where(['follow_unfollows.follower_id'=> $request->user_id,'type' => 0]);
            }
            $followerData = $query->groupBy('user_id')->get();
        ...
        try{
         $validator = Validator::make($request->all(), [
            ...
            if(!empty($request->search)){
             ...
             $query->where(['follow_unfollows.user_id'=> $request->user_id,'status' => 1,'type' =>0]);
            }
            $response = [];
            ...
            if(count($followingData) > 0){
             foreach($followingData as $dt){
                ...
                $data['following_id'] = $dt->follower_id;
                $data['name']  = $dt->name;
                $data['image']  = 'uploads/profile/'.$dt->image;
                $data['email'] = $dt->email;
                $data['address'] = $dt->address;
    ...
    public function comment(Request $request){
     try{
       $validator = Validator::make($request->all(), [
        ...
        $getUserName = User::select('name')->where('id',$request->user_id)->first();
        $getPostUser_id = Post::select('user_id')->where('id',$request->post_id)->first();
        ...
        if(!empty($checkPostId)){
             ...
             $getDeviceID = User::select('device_id')->where('id',$checkPostId->user_id)->first();
                     if(!empty($getDeviceID)){
                         if($getPostUser_id->user_id != $request->user_id){
                        $title =  $getUserName->name . ' commented on your post';
                        $messageData['title']           = $title;
                        $messageData['message']         = $getUserName->name .' '.$this->notificationMessage('3');    
                        $messageData['user_id']         = $request->user_id;
                        $messageData['id']              = $request->post_id;
        ...
        if ($validator->fails()) {
         ...
         if(is_null($checkId)){
          ...
          $getDeviceID = User::select('device_id')->where('id',$getUserIdOfComment->user_id)->first();
                     if(!empty($getDeviceID)){
                        if($getUserIdOfComment->user_id != $request->user_id){
...
function getCountOfFollowerAndFollowing($type , $user_id){
    if($type == 1){
         $data = DB::table('follow_unfollows')->where(['follow_unfollows.follower_id'=> $user_id,                     'type' => 0])->count();
        return $data;
    }
    else{
  ...
  public function notificationList(Request $request){
    try{
      ...
      $query = DB::table('notifications')->select('notifications.id','notifiable_type','data','sender_id'             ,'receiver_id','notifiable_id','name as receiver_name','image','email','address','phone',              'business_name','users.created_at','place','follow_unfollows.status')
                      ->leftJoin('users','users.id' ,'=', 'notifications.sender_id')
                      ->leftJoin('follow_unfollows','follow_unfollows.id','=','notifications.notifiable_id');
       if($request->type == 1){
         $query->where(['receiver_id' => $request->user_id,'notifiable_type' => $request->type]);
        } 
        elseif($request->type == 'all'){
         $query->where('notifiable_type', '!=' , '1')
    ...
    if(count($getNotification) > 0){
        ...
        $messaage = json_decode($dt->data, true);
         ...
         $data['notifiable_type'] = $dt->notifiable_type;
  ...
  public function followBack(Request $request){
    try{
      $validator = Validator::make($request->all(), [
        'user_id' => ['required','exists:follow_unfollows,user_id'],
        'follower_id' => ['required','exists:follow_unfollows,follower_id'],
        'status' => ['required','exists:follow_unfollows,status'],
       ]);
       if ($validator->fails()) {
        return response()->json(['status'=>false,'message' => $validator->messages()->first()], 200);
       } 
    ...
    $data = DB::table('follow_unfollows')->insert($arr);
    if($data){
      return response()->json = [
           'status' => true,
  ...
  public function cancelRequest(Request $request){
    try{
      $validator = Validator::make($request->all(), [
        'follow_unfollow_id' => ['required','exists:follow_unfollows,id'],
       ]);
       if ($validator->fails()) {
        return response()->json(['status'=>false,'message' => $validator->messages()->first()], 200);
       } 
       $cancelRequest = DB::table('follow_unfollows')->where('id' , $request->follow_unfollow_id)->delete();
        $deleteNotification = DB::table('notifications')->where(['notifiable_id' => $request->follow_unfollow_id , 'notifiable_type' => 1])->delete();
        if($cancelRequest){
        return response()->json = [
           'status' => true,
